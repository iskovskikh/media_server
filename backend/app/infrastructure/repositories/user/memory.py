...
class MemoryUserRepository(BaseUserRepository):
    ...
    async def update_user(self, user: User) -> None:
        for i, item in enumerate(self._saved_user_list):
            if user.oid == item.oid:
                self._saved_user_list[i] = user
    ...
    async def get_user_by_login(self, login: str) -> User | None:
        try:
            return next(
                user for user in self._saved_user_list if user.login == login
            )
        except StopIteration:
            return None
